...
public class CalendarDateDemo {
    ...
    /**
     * Main method to demonstrate CalendarDate functionality.
     * 
    ...
    public static void main(String[] args) {
        ...
        System.out.println("There are " + CalendarDateDemo.countDatesBefore(classMeetings, new CalendarDate(10, 15, 2025)) +
                " class meetings before the midterm exam.");
        // Expected output: There are 14 class meetings before the midterm exam.
    }
    
    /**
     * Counts the number of dates in the given array that come before the given target date.
     * 
     * @param dates the array of CalendarDate objects to check
     * @param target the target date to compare against
     * @return the number of dates in the array that come before the target date
     */
public static int countDatesBefore(CalendarDate[] dates, CalendarDate target) {
        ...
        for (CalendarDate date : dates) {
            // Check for null elements and compare with target date
            if (date != null && date.comesBefore(target)) {
                count++;  // Increment count for dates before target
            }
        }
