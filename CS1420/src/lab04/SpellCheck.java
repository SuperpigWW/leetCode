package lab04;

import java.io.File;
import java.io.FileNotFoundException;
import java.util.Scanner;

public class SpellCheck {
	
	/**
	 * Given a filename, this method returns a count of the number of
	 * words in the file. If the file cannot be opened, -1 is returned.
	 * 
	 * @param the name with path of a text file
	 * @return the count of words in the file or -1
	 */
	public static int countWords(File file) {
        int count = 0;
        try {
            Scanner fileInput = new Scanner(file);
            while (fileInput.hasNext()) {
                fileInput.next(); 
                count++;
            }
            fileInput.close();
        } catch (FileNotFoundException e) {
            return -1;
        }
        return count;
    }
	
	public static String getClosestWord(String word, File file) {
		String current = null;
		try {
			Scanner fileInput = new Scanner(file);;
			while(fileInput.hasNext()) {
				//fileInput = new Scanner(file);
				current = fileInput.next();
				if(word.compareTo(current) <= 0)
				    break;
			}
			fileInput.close();
		}catch (FileNotFoundException e) {
            
        }
		return current;
	}
	
	public static void spellCheck(File file) {
		while (true){
			Scanner input = new Scanner(System.in);
			System.out.printf("Enter a word to check:%n");
			String wordIn = input.nextLine();
			if (wordIn.equals(getClosestWord(wordIn, file)))
				System.out.printf("%s is in the list.%n",wordIn);
			else {
				System.out.printf("%s is not din the list.%n",wordIn);
				System.out.printf("%s is in the list.%n",getClosestWord(wordIn, file));
			}
		}
	}

	public static void main(String[] args) {
		File file = new File("C:\\Users\\86158\\Desktop\\words.txt");
		int a = countWords(file);
		String b = getClosestWord("goood", file);
		System.out.printf("%d%n%s%n",a,b);
		spellCheck(file);

	}

}
