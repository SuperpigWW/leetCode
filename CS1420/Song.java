...
public class Song {
	...
	public void addNoteEvent(int time, int trackNumber, int duration, int pitch) {
		NoteEvent noteEventNew = new NoteEvent(time, trackNumber, duration, pitch);
		tracks[trackNumber].add(noteEventNew);
	...
	public void addVolumeEvent(int time, int trackNumber, int value) {
		VolumeEvent VolumeEventNew = new VolumeEvent(time, trackNumber, value);
		tracks[trackNumber].add(VolumeEventNew);
		Collections.sort(tracks[trackNumber]);
	}
	...
	public void clearTrack(int trackNumber) {
		if (trackNumber < 0 || trackNumber > 9)
            throw new IllegalArgumentException ("TrackNumber out of range.");
